...
class PolicyReinforcementPPODiscrete(PolicyBase):
    ...
    def __init__(self, execution, contract_manager, account_manager, args):
        ...
        args.action_dim = 255
        self.int_agent = PPO(state_dim=110+ACTION_SIZE, action_dim=255, lr_actor=lr_actor, lr_critic=lr_critic, gamma=gamma, K_epochs=K_epochs, eps_clip=eps_clip, has_continuous_action_space=has_continuous_action_space, action_std_init=action_std)
        args.action_dim = 257
        self.uint_agent = PPO(state_dim=110+ACTION_SIZE, action_dim=257, lr_actor=lr_actor, lr_critic=lr_critic, gamma=gamma, K_epochs=K_epochs, eps_clip=eps_clip, has_continuous_action_space=has_continuous_action_space, action_std_init=action_std)
    ...
    def _select_int(self, contract, method, size, obs, chosen_int, x_state, hidden, episole):
        ...
        action = self.int_agent.select_action(x_state)
    ...
    def _select_uint(self, contract, method, size, obs, chosen_int, x_state, hidden, episole):
        valid_action_indices = list(range(0, size+1))
        action = self.uint_agent.select_action(x_state)
